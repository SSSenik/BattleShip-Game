	...
	for(p=0; p<PLAYERS; p++) {
	    for(s=0; s<SHIPNUM; s++) {
			...
			for(iRes=0; iRes<GSIZE; iRes++) {
				printf("  %d ", iRes);
				for(jRes=0; jRes<GSIZE; jRes++){
					printf("%c ", gmPlayer[p].result[iRes][jRes] ? ONTARGET : ONWATER);
				}
				printf("\n");
			}
			res = 0;
...
void showGameField(stt_gamePlayer gmPlayer[], int p) {
	...
	for(iCur=0; iCur<GSIZE; iCur++) {
		printf("  %d ", iCur);
		for(jCur=0; jCur<GSIZE; jCur++){
			printf("%c ", gmPlayer[p].current[iCur][jCur]);
		}
		printf("\n");
	}
	printf("\nWater -> %c\n", ONWATER);
...
int setShipsPl(stt_gamePlayer gmPlayer[], int p, int s) {
    ...
    x = gmPlayer[p].ships[s].sh_cords[0]-48;
    y = gmPlayer[p].ships[s].sh_cords[1]-97;
	
    for(i=0; i<gmPlayer[p].ships[s].sh_size; i++) {
        if(gmPlayer[p].ships[s].sh_ori == 'v') {
        ...
        } else if(gmPlayer[p].ships[s].sh_ori == 'h') {
